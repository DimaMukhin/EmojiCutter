...
class Cutter extends Component {
  ...
  fileUploadHandler = async () => {
    ...
    try {
      let res = await emojiCutterClient.cutImageToLargeEmoji(
        this.state.selectedFile,
        this.state.emojiName,
        this.updateUploadProgressHandler
      );
      this.props.setEmojiString(res.data.emojiString);
      this.setState({
        largeEmojiFileName: res.data.fileName,
        downloadReady: true
      });
    } catch (err) {
      ...
      if (err && err.response && err.response.data && err.response.data.code)
        errorMessage = ServerErrorHelper.getErrorMessage(
  ...
  fileDownloadHandler = () => {
    if (!this.state.largeEmojiFileName) return;

    emojiCutterClient.downloadEmojiInNewWindow(this.state.largeEmojiFileName);
  };

  render() {
    const uploadFileLabel = this.state.selectedFile
      ? StringHelper.truncate(this.state.selectedFile.name, 20)
      : 'Select file';
    let progressMessage =
      !this.state.downloadReady &&
      this.state.fileUploadPercent === 100 &&
            ...
            <div style={styles.genDownButtonsContainer}>
              <div>
                <Button
                  ...
                  disabled={!this.state.selectedFile || !this.state.emojiName}
                >
                  Generate
                </Button>
              </div>
              <div>
                <Button
                  onClick={this.fileDownloadHandler}
